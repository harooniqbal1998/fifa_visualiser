...
import { ACTUAL_RESULTS } from "../src/data/actual-results";
...
function resultsEqual(
  a: Record<string, MatchResult>,
  b: Record<string, MatchResult>,
): boolean {
  ...
  return keysA.every(
    (key, index) =>
      key === keysB[index] &&
      a[key].homeScore === b[key].homeScore &&
      a[key].awayScore === b[key].awayScore,
  );
}
...
async function main() {
  ...
  const existingResults = existsSync(OUTPUT_PATH) ? ACTUAL_RESULTS : null;
  if (existingResults && resultsEqual(results, existingResults)) {
    console.log(
      `No changes (${Object.keys(results).length} results); skipped writing ${OUTPUT_PATH}`,
    );
  } else {
